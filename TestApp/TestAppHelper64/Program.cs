 ...
 * 
 * 
 * TestAppHelper64 - small command-line helper used by TestApp.
 *
 * TestApp is a 64-bit process, but ClrMD must load a DAC that matches
 * the target's architecture. When the target is a 64-bit .NET
 * process and the direct in-process read fails for any reason (for
 * example a cross-session attach, or a mismatch between the ClrMD
 * version the main process loaded and the target's runtime), TestApp
 * falls back to launching this helper. This helper is compiled as
 * x64, so it can load the correct 64-bit DAC and inspect the target
 * from a separate, clean process.
 *
 * Usage: TestAppHelper64.exe <pid> <tid> <outputFilePath>
 * Exit codes:
