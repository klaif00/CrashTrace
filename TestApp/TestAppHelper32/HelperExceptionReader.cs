 ...
 * 32-bit .NET process. Runs inside the x86 helper, so the ClrMD DAC
...
namespace TestAppHelper32
{
	...
	{
		// Same algorithm as ManagedExceptionReader.TryReadDirect, but
		// compiled as x86 so the DAC that ClrMD loads matches a 32-bit
		// target. All out parameters are initialized before any early
		// return.
